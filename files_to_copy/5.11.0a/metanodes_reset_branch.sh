...
reset_notification_id=$(
    notify-send --print-id --urgency=critical --expire-time=0 \
    	--icon=software-update-available \
        "${repo} repository" \
        "<b>Repository (${branch} branch) reset started!</b> \
        \rPlease wait until it is finished..."
)
...
if [ $? != 0 ] ; then
  ...
  notify-send -u critical -i dialog-error --expire-time=0 --replace-id="${reset_notification_id}" \
  "${repo} repository" \
...
notify-send -u critical -i process-completed --expire-time=0 --replace-id="${reset_notification_id}" \
"${repo} repository" \
